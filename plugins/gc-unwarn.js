...
const handler = async (m, {conn, text, command, usedPrefix}) => {
  ...
  const warntext = `*اعمل تاج او منشن لي الشخص الي تريد ترفع عنه الانذار*\n*مثال:* \n*${usedPrefix + command} @${global.suittag}*`;
  if (!who) throw m.reply(warntext, m.chat, {mentions: conn.parseMention(warntext)});
  if (m.mentionedJid.includes(conn.user.jid)) return;
  if (user.warn == 0) throw tradutor.texto2;
  user.warn -= 1;
  await m.reply(`${user.warn == 1 ? `*@${who.split`@`[0]}*` : `♻️ @${who.split`@`[0]}`} *تم الغاء الانذار عنك خليك ر​​​​​جال  لو عملت غلط ثاني بفشخك*\n*عدد الانذارات المتبقية عليك ${user.warn}/3*`, null, {mentions: [who]});
};
